...
export const actions: Actions = {
	updateProfile: async ({ request, locals }) => {
		...
		if (!result.success)
			return fail(400, { action: 'profile', errors: z.flattenError(result.error).fieldErrors });

		const res = await updateProfile(locals.user.id, result.data);
		if (!res.ok) return fail(500, { action: 'profile', errors: res.errors });
	...
	updatePassword: async ({ request }) => {
		...
		if (!result.success)
			return fail(400, { action: 'password', errors: z.flattenError(result.error).fieldErrors });

		const res = await changePassword(request.headers, result.data);
		if (!res.ok) return fail(400, { action: 'password', errors: res.errors });
	...
	createUser: async ({ request, locals }) => {
		...
		if (!result.success)
			return fail(400, { action: 'create', errors: z.flattenError(result.error).fieldErrors });

		const res = await createUser(request.headers, result.data);
		if (!res.ok) return fail(400, { action: 'create', errors: res.errors });
	...
	deleteUser: async ({ request, locals }) => {
		...
		if (!res.ok) return fail(500, { action: 'delete', errors: res.errors });
